...
#### 3.3 说明：不输出“建议%趋势类指标”
为避免“方向信号”和“建议占比变化”口径互相打架，本周报不再输出以下指标：
- 周内建议%范围
- 周初/周末中位数%
- 周初→周末变化（Δ）
- 基于 Δ 的上行/下行/稳定/震荡趋势

#### 3.6 本周“增持/减持”信号（全周方向汇总）
目标：回答“这一周里所有投资建议综合起来，我的标的到底偏向增持还是减持”。

对某一行（大板块或标的）：
- 使用“3.1 日内主方向”得到该行周内每一天的主方向序列（增 / 减 / 不变 / 无明显偏向）。
- 计数（按天）：
  - `增天`：主方向为“增”的天数
  - `减天`：主方向为“减”的天数
  - `不变天`：主方向为“不变”的天数
  - `有效天`：`增天+减天+不变天`（忽略 `无明显偏向` 与缺失）
- 信号分数（用于强度与趋势计算）：`score = (增天 - 减天) / 有效天`，范围 `[-1, 1]`；若 `有效天=0` 则记为缺失。
- 本周动作建议（信号汇总）：
  - 若信号分数缺失：写 `—`
  - 大板块：
    - `score >= +0.20` → `增配`
  ...
  - 标的：
    ...
    - `score <= -0.20` → `减持`
    - 其他 → `维持`
- 信号强度（全周）：`abs(score)`，展示为百分比（两位小数），例如 `66.67%`；若缺失写 `—`。

#### 3.7 信号变化（变强/变弱/反转/稳定）
目标：回答“增持/减持这个信号在这一周内是在变强、变弱，还是发生方向反转”。

对某一行（大板块或标的）：
- 设 `n` 为纳入统计的日报数量；取 `k = max(1, floor(n/2))`。
- 前半段：取最早的 `k` 天；后半段：取最晚的 `k` 天。
- 分别按“3.6”的口径计算前半段与后半段的 `score` 与 `strength=abs(score)`。
- 趋势判断：
  - 若前半段或后半段 `score` 缺失：写 `—`
  - 若前后两段动作建议方向相反，且均不为 `维持`：写 `反转：<前半段动作>→<后半段动作>`（例如 `反转：增持→减持`）
  - 若 `后半段 strength - 前半段 strength >= 0.20`：写 `变强`
  - 若 `前半段 strength - 后半段 strength >= 0.20`：写 `变弱`
  - 其他：写 `稳定`

#### 3.8 方向序列（↑↓-）
目标：按天序列展示周内主方向的“顺序”，便于快速看一周内节奏。

对某一行（大板块或标的）：
- 使用“3.1 日内主方向”得到该行周内每一天的主方向序列（与周报纳入的日报日期顺序一致）。
- 映射规则：
  - 主方向为“增”（大板块为 `增配` / 标的为 `增持`）→ `↑`
  - 主方向为“减”（大板块为 `减配` / 标的为 `减持`）→ `↓`
  - 其他情况（`不变` / `无明显偏向` / 缺失）→ `-`
- 输出为一个连续字符串，例如：`↑↑-↓--↑`；若无有效天则写 `—`。
...
- 三级标题：`### 信号聚焦`
- 输出两张表：
  1) `信号最强 TOP10`：按 `信号强度（全周）` 从大到小，列：`标的/大板块`、`周内方向统计（按天）`、`本周动作建议（信号汇总）`、`信号强度（全周）`、`信号变化`
  2) `信号变化最大 TOP10`：按“信号变化幅度”从大到小（用于排序，表中不强制展示数值），列同上
...
- 大板块与标的两张趋势表中，每行均包含：周内方向统计、本周动作建议（信号汇总）、信号强度（全周）、信号变化、备注。
- 大板块与标的两张趋势表中，必须包含“方向序列(↑↓-)”列，且与纳入日报日期顺序一致。
- 百分比全部为两位小数并带 `%`。
- 不出现任何来自输入集外的内容，不出现推测与新增结论。

## GitHub 上传（每周总结完成后必须执行）
- 在成功写入 `每周分析报告/<WEEK_START>_to_<WEEK_END>_每周投资总结.md` 后，执行：
  - `git add "每周分析报告/<WEEK_START>_to_<WEEK_END>_每周投资总结.md"`
  - `git commit -m "每周总结 <WEEK_START>_to_<WEEK_END>"`
  - `git push`
