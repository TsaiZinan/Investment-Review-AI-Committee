...
## 生成规则（严格执行）
### -1) 标的与数据源一致性校验（必须先执行）
- 数据源固定为：`报告/<DATE>/投资策略.json`（只读，不可改写）。
- 你必须在读取并汇总各模型报告之前，先对每一份 `^<DATE>_.+_投资建议\.md$` 执行“标的核对”：
  - 从数据源 `investment_plan[*].fund_name` 得到“应出现的标的清单”（数据源标的集合）。
  - 从该模型报告的“定投计划逐项建议（全量，逐项表格）”表格读取“标的”列，得到“该报告实际标的清单”（报告标的集合）。
  - 判定一致性（必须严格输出差异）：
    - `报告多出`：在报告标的集合里，但不在数据源标的集合里
    - `报告缺失`：在数据源标的集合里，但不在报告标的集合里
- 若任意报告存在 `报告多出` 或 `报告缺失`（或报告无法解析出逐项表格导致无法核对）：
  - 你必须在对话中输出“标的核对结果”，并清晰列出每份报告的差异清单（建议用 Markdown 表格：文件名 / 报告多出 / 报告缺失）。
  - 不要生成或写入 `每日最终报告/<DATE>_最终投资总结.md`。
  - 明确告知用户：需要用户决定下一步（修正报告、更新数据源、或确认忽略差异并继续）。
...
2. 新增：定投增减要点综述（必须，1000字以内，单段落）
- 二级标题：`## 0. 定投增减要点综述（1000字以内）`
- 内容要求：
  - 必须只基于各模型报告里的 `### 1. 定投增减要点（最多 5 条）` 这一段（或同义标题）做总结
  - 汇总当天所有报告的“增持/减持/暂停/维持”等倾向，强调重复出现频率最高的要点，并点出显著分歧（如同一标的出现增持与减持并存）
  - 严格一段话，不要分点，不要换行，不要加入目录外信息
  - 总长度不超过 1000 个中文字符（含标点）

3. 第一部分：大板块比例调整建议横向对比
...
## GitHub 上传（每日总结完成后必须执行）
- 在成功写入 `每日最终报告/<DATE>_最终投资总结.md` 后，执行：
  - `git add "每日最终报告/<DATE>_最终投资总结.md"`
  - `git commit -m "每日总结 <DATE>"`
  - `git push`
