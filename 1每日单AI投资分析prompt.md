...
0. 确认“模型名”（必须执行）：
   - 模型名用于所有需要 `{模型名}` 的文件名与路径片段，必须逐字一致
   - 如果你无法从运行环境可靠获取模型名：必须在开始前向用户索取一个“模型名字符串”（例如：`GPT-5.2`），并以该字符串为准继续执行；不得自行猜测或使用空值
...
- 保存后：必须执行“最终校验与清理”（本地工作流第 5 步），并在进度文件中写明校验结果与清理记录
