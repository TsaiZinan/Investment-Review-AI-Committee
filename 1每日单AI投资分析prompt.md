...
0. 确认“模型名”（必须执行）：
   ...
   - 模型名候选列表（可增补）：DeepSeek, Gemini, GPT-5.2, Grok-4, GLM-4.7, GLM-5, Kimi, Kimi-K2.5, MiniMax-M2.1, MiniMax-M2.5, TraeAI, Germini-3-Pro
   - 若用户输入的模型名不在候选列表：必须将该模型名追加到本文件的“模型名候选列表（可增补）”中（去重、保持原样），再继续后续步骤
