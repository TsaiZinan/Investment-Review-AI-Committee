...
0. 确认“模型名”（必须执行）：
   ...
   - 在提问前，必须先运行：`python scripts/model_registry.py suggest --date "{YYYY-MM-DD}"`
   - 候选列表不再手工维护，必须以 `Data/model_registry.json` + 历史报告扫描结果为准，并引用上一条命令输出的“动态候选（按最近使用生成）”作为建议项
   - 用户回复后，必须立即运行：`python scripts/model_registry.py observe --model "{模型名}" --date "{YYYY-MM-DD}"`，把这次使用登记到模型注册表
   - 若用户输入的模型名不在动态候选中：直接接受该输入，不要改写本 Prompt；只需通过上一条 `observe` 命令登记该模型名即可
